...
my $fileList = "list_methabolic_pathways.txt";
...
foreach $term (@res){
	...
	my $query = "http://www.uniprot.org/uniprot/?query=organism:$taxon+AND+$term&columns=id,entry name,reviewed,protein names,genes,organism,length,pathway&format=xls";
	my $file = "uniprot_" . $taxon . "_". $term . "_EXCEL.xls";
